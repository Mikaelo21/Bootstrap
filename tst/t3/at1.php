...
<body style="user-select: none !important;" class="bg-info d-flex flex-column align-items-center justify-content-center vh-100"> <!--bg-info(deixa o fundo com cor azul claro) d-flex(deixa responsivo tudo que ta dentro da dfiv, o alig item e justify content só alinha os itens no meio, vh-100 é pora definir quando começa a div de cima para baixo) -->
  ...
  <h2 class="h4 text-primary mb-3">Tabuada</h2> <!--h4(tamanho do texto), text-primary(deixa a cor do texto azul) e mb-3 ajusta o tamanho do h2 -->
        <form action="1.php" method="post">  <!--tag de form que envia os dados para o 1.php-->
    <div class="mb-3"> 
     <label class="form-label text-secondary">Digite um número:</label> <!--label do form com a classe de form label do bootstrap e a cor text-secondary(cinza) -->
         <input type="number" name="numero" required class="form-control"> <!--cria um input obrigatorio com a classe de form do bootstrap -->
      </div>
     ...
     <a href="index.php" class="btn btn-outline-danger">Voltar ao inicio</a><!--tag de link com classe de botao do bootsrap com animação, leva de volta ao index.php -->
        <button type="submit" class="btn btn-outline-success">Calcular</button> <!--tag de botao com classe de botao do bootsrap que leva pro 1.php -->
        </form>
    </div>
</body>
</html>